...
export function ShippingEstimate({
  ...
  const [shippingRatePerKg, setShippingRatePerKg] = useState<number | null>(
    null,
  );
  ...
  const [courierPriceResult, setCourierPriceResult] =
    useState<PriceCalculationResult | null>(null);

  // Receiving country pricing state (for IN, MY, LK, AE, GB destinations)
  const [exchangeRatePrice, setExchangeRatePrice] = useState<number | null>(
    null,
  );
  ...
  const [convertedPrices, setConvertedPrices] = useState<
    Map<string, { sourcePrice: number | null; destinationPrice: number | null }>
  >(new Map());
  ...
  useEffect(() => {
    const calculateCourierPrices = async () => {
      ...
      try {
        ...
        const fromCountry =
          shipmentType === "export" ? originCode : destinationCountryCode;
        const toCountry =
          shipmentType === "export" ? destinationCountryCode : originCode;
  ...
  useEffect(() => {
    const convertPrices = async () => {
      ...
      const newConvertedPrices = new Map<
        string,
        { sourcePrice: number | null; destinationPrice: number | null }
      >();
  ...
  const formatCurrencyAmount = useCallback(
    (amount: number, currencyCode: string = destinationCurrency) => {
  ...
  const calculateShippingEstimate = () => {
    ...
    const weightBasedInternational =
      weightValue && weightValue > 0 ? weightValue * shippingRatePerKg : 0;

    // Calculate volume-based international shipping (for display purposes)
    const volumeBasedInternational =
      volumeWeight && volumeWeight > 0 ? volumeWeight * shippingRatePerKg : 0;
    ...
    const serviceCharge =
      (internationalShipping + priceCalculatorTotalLKR) *
      (serviceChargePercent / 100);
    const internationalShippingWithService =
      internationalShipping + serviceCharge;

    // Domestic Shipping in destination country (only if delivery)
    const domesticShippingAmount =
      deliveryOption === "delivery" ? domesticShipping : 0;

    // Shipping Total = International Shipping (with service charge) + Domestic Shipping
    const shippingTotal =
      internationalShippingWithService + domesticShippingAmount;
    ...
    <Card className="w-full border-dashed border-2">
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground mt-2">
          Optional: Get an estimate of international shipping costs based on
      ...
      {isExpanded && (
        <CardContent className="space-y-4">
                    ...
                    <CardContent className="pt-4">
                      <div className="space-y-2">
                        ...
                        <div className="text-sm space-y-1">
                          ...
                          <p>
                            No.4, COUNCIL AVENUE, (NEAR DEHIWALA MUNICIPAL
                            COUNCIL)
                          </p>
              ...
              <div className="mt-4 space-y-4">
                ...
                <div className="space-y-3">
                    ...
                    <CardContent className="p-6 flex flex-col items-center">
                      <div className="w-full">
                        ...
                        <div className="text-center mb-2">
                          ...
                          <p className="text-sm text-muted-foreground mt-2">
                            Effective Weight:{" "}
                            {formatNumber(calculateEffectiveWeight() || 0, 2)}{" "}
                            kg
                          </p>
                        </div>
                      </div>
                  ...
                  <div className="text-xs text-muted-foreground mt-2 pt-2 border-t">
                    <p>
                      <strong>Note:</strong> Prices are calculated based on
                      weight using receiving country rates. All prices are in
                      INR and are estimates. Final charges may vary after
            ...
            sortedCourierPrices.length > 0 && (
              <div className="mt-4 space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  >
                    {sortedCourierPrices.map((servicePrice, index) => {
                      ...
                      const sourceCurrencyCode =
                        countryCodeToCurrencies(originCode);
                      const destinationCurrencyCode = countryCodeToCurrencies(
                        destinationCountryCode,
                      );
                      ...
                      const destinationPrice =
                        convertedPrice?.destinationPrice ??
                        servicePrice.finalPrice ??
                        0;
                            ...
                            <div className="flex flex-col gap-2 flex-1 relative">
                              ...
                              {servicePrice.final_weight && (
                                <div className="text-xs text-muted-foreground text-center">
                                  Weight:{" "}
                                  {formatNumber(
                                    servicePrice.final_weight / 1000,
                                    2,
                                  )}{" "}
                                  kg
                                </div>
                  ...
                  <div className="text-xs text-muted-foreground mt-2 pt-2 border-t">
                    <p>
                      <strong>Note:</strong> Prices are calculated based on the
                      higher of actual or volumetric weight. All prices are
                      estimates. Final charges may vary after courier
                      verification.
                    </p>
                  </div>
                </div>
          ...
          {!shouldUseExchangeRate &&
            ...
            sortedCourierPrices.length === 0 && (
              <div className="text-sm text-muted-foreground text-center py-4">
                No courier services available for this route and weight. Please
                check your inputs.
              </div>
            )}
          ...
          {shouldUseExchangeRate &&
            ...
            exchangeRatePrice === null && (
              <div className="text-sm text-muted-foreground text-center py-4">
                Unable to calculate shipping price. Please check your inputs or
                try again later.
              </div>
            )}
